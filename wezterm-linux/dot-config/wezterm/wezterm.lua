...
config = {
	...
	keys = {
		{ key = "q", mods = "SUPER|SHIFT", action = wezterm.action.QuitApplication },
		...
		{ key = "Home", action = wezterm.action({ SendString = "\x01" }) },
		{ key = "End", action = wezterm.action({ SendString = "\x05" }) },
		...
		{ key = "z", mods = "CTRL", action = wezterm.action({ SendString = "\x1f" }) },
